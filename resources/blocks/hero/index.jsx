...
const HeroContent = ({ attributes }) => {
  ...
  return (
    <header id="home" className="twst-hero relative w-full scroll-mt-32 overflow-hidden px-6 py-20 md:py-28">
      ...
      <div className="relative z-10 mx-auto max-w-6xl text-center">
        <h1 className="mx-auto max-w-5xl text-balance text-4xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100 md:text-7xl">{headline}</h1>
        <p className="mx-auto mt-8 max-w-4xl text-pretty text-lg leading-relaxed text-zinc-500 dark:text-zinc-400 md:text-2xl">{description}</p>
        <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
          <a className="twst-btn-accent" href={attributes.primaryUrl || '#'}>{primaryLabel}</a>
          <a className="inline-flex items-center justify-center rounded-2xl bg-zinc-200 px-8 py-4 text-base font-semibold text-zinc-900 transition hover:bg-zinc-300 dark:bg-zinc-800 dark:text-zinc-100 dark:hover:bg-zinc-700" href={attributes.secondaryUrl || '#'}>{secondaryLabel}</a>
        </div>
      </div>
    </header>
  );
      ...
      <>
        <InspectorControls>
          <PanelBody title={__('Content', 'sage')} initialOpen>
            ...
            />
            <p className="components-base-control__label">{__('Primary CTA URL', 'sage')}</p>
            <URLInputButton
            ...
            />
            <p className="components-base-control__label">{__('Secondary CTA URL', 'sage')}</p>
            <URLInputButton
