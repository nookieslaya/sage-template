...
const BlogPreviewContent = ({ attributes }) => {
  ...
  return (
    <section className="mx-auto max-w-7xl scroll-mt-32 px-6 py-20 md:py-24" id="blog">
      ...
      <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {[1, 2, 3].map((index) => (
          <article key={`placeholder-${index}`} className="rounded-3xl border border-dashed border-zinc-300 bg-zinc-50 p-8 text-zinc-500 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-400">
            {__('Latest post #', 'sage')}{index}{__(' will render on frontend', 'sage')}
            <div className="mt-3 text-sm">{postLinkLabel}</div>
          </article>
        ))}
      </div>
      <div className="mt-10 text-center"><span className="twst-btn-accent">{viewMoreLabel}</span></div>
    </section>
  );
};
